...
>>>>>>> c54fde121c99ea8d10fddcc030fc848ef4ac9610
  return (
    <section id="first-screen">
      <div className="tabs">
        <h2 className={`tab-title animate-opacity ${activeTab === 'stories' ? 'active' : ''}`}>
          Explore climate stories
          <br/>
          anywhere in the world
        </h2>
        <h2 className={`tab-title animate-opacity ${activeTab === 'map' ? 'active' : ''}`}>
          Discover similar places
          <br/>
          and climate actions happened there
        </h2>
        <h2 className={`tab-title animate-opacity ${activeTab === 'research' ? 'active' : ''}`}>
        ...
        <div
          id="stories-tab"
          className={`tab-container animate-opacity ${activeTab === 'stories' ? 'active' : ''}`}
        >
          ...
          <div className="tab-description">
            ...
            <p>Climate change impacts our planet and the lives of people. We are here to help with our research platform.</p>
          </div>
        </div>
        <div
          id="map-tab"
          className={`tab-container animate-opacity ${activeTab === 'map' ? 'active' : ''}`}
        >
          <div className="tab-input">
            <input
              type="text"
              ...
              {isLoading ? (
                ...
                <>
                  {placeSuggestions.map(place => {
                    const { place_id, name, state_name, nation_name } = place.properties;
                    return (
                      <p
                        key={place_id}
                        onClick={() => {
          ...
          <div className="tab-description">
            ...
            <p>We will find similar places based on climate parameters and you can navigate to find climate solutions happened in those places.</p>
          </div>
        </div>
        <div
          id="research-tab"
          className={`tab-container animate-opacity ${activeTab === 'research' ? 'active' : ''}`}
        >
          <div className="tab-input">
            <input
              type="text"
              ...
              onKeyDown={event => {
                if (event.key === 'Enter') {
                  window.open(`https://www.google.com/search?q=${researchSearchTerm}`);
                }
              }}
            />
            {!isLoading && (
          ...
          <div className="tab-description">
            ...
            <p>Investigate climate actions on the web and ClimateTree will help to share with the world.</p>
          </div>
        </div>
      </div>
